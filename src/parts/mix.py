...
def execute(args):
    ...
    if not os.path.exists(os.path.join(args.output_dir, DirName.POSETRIPLET.value)):
        logger.error(
            "指定されたPoseTripletディレクトリが存在しません。\nPoseTripletが完了していない可能性があります。: {img_dir}",
            img_dir=os.path.join(args.output_dir, DirName.POSETRIPLET.value),
            decoration=MLogger.DECORATION_BOX,
        )
        raise MApplicationException()

    try:
        output_dir_path = os.path.join(args.output_dir, DirName.MIX.value)
        ...
        for personal_json_path in tqdm(sorted(glob(os.path.join(args.output_dir, DirName.POSETRIPLET.value, "*.json")))):
        ...
        for personal_json_path in sorted(glob(os.path.join(args.output_dir, DirName.POSETRIPLET.value, "*.json"))):
            pname, _ = os.path.splitext(os.path.basename(personal_json_path))
